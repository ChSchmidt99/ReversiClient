...
bool isValidMove(Field* field, char own, unsigned int x, unsigned int y) {
    ...
    for(int i = (int)y; i < field->height; i++) {
        char column = getColumn(field, x, i);
    ...
    // horizontal check >
    for(int j = (int)x; j < (int)field->width; j++) {
        char column = getColumn(field, j, y);
    ...
    // diagonal check <^
    for(int i = (int)y, j = (int)x; i >= 0 && j >= 0; j--, i--) {
    ...
    // diagonal check |>
    for(int i = (int)y, j = (int)x; i < field->height && j < field->width; j++, i++) {
        char column = getColumn(field, j, i);
